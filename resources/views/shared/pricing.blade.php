...
<div class="card border-0 shadow-sm mt-5">
    <div class="card-body p-4 p-md-5">
        <div class="text-uppercase font-weight-bold small mt-3 mb-3 text-primary">{{ __('Upgrade Options') }}</div>
        @foreach($plans as $plan)
        @php $value = 0; @endphp
        <div class="form-check">
            <input class="form-check-input" type="radio" name="flexRadioDefault" value="{{$loop->index}}" id={{$loop -> index}} @if($loop->index == 0) checked @endif >
            <label class="form-check-label" for={{$loop -> index}}>
            {{ $plan -> name}} ${{ $plan -> amount_month}} / mo 
            </label>
        </div>
        @endforeach
...
<div class="d-flex justify-content-center">
    @foreach($plans as $plan)
        <div class="mt-4 mx-2 plan-toggle plan-toggle{{ $loop->index }} @if($loop->index != $value) d-none @endif">
            @auth
                @if($plan->hasPrice())
                    @if(Auth::user()->plan->id == $plan->id)
                        <div class="btn btn-lg btn-primary font-size-lg disabled">{{ __('Active') }}</div>
